...
class Lane:
  ...
  def __init__(self, orig_frame,cross_signal=False):
    ...
    self.YM_PER_PIX = 0.7 / 300 # meters per pixel in y dimension
...
# Blend filled polygons with the original image
  ...
  def relay(self):
    if self.b_left is not None:
        if self.b_left < -0.15 and self.b_left > -0.4:
            self.steer = 15
        elif self.b_left > 0.15 and self.b_left < 0.4:
            self.steer = -15
    if self.b_right is not None:
        if self.b_right < -0.15 and self.b_right > -0.4:
            self.steer = 15
        elif self.b_right > 0.15 and self.b_right < 0.4:
            self.steer = -15
    # return 0
  def run(self):
    ...
    if raddii is not None:  
      ...
      if self.steer > -6 and self.steer < 6:
    	  self.relay()
    ...
    return self.frame_with_lane_lines2,self.steer
